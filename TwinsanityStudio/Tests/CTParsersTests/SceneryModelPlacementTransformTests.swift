...
/// directions (this file's own history: three earlier versions of both
/// `worldTransform` and this exact helper independently made matching
/// mistakes — a straight row-to-column copy, then a partial transpose that
/// negated the wrong components, then a *full* transpose that reproduced
/// the reference source's own arithmetic bit-for-bit and was still 180°
/// off for real, non-symmetric geometry, confirmed against the actual
/// reference tool's live rendering, not just its source — so encode/decode
/// agreed with each other, and even matched hand-traced source arithmetic,
/// while still disagreeing with real rendered output; every test here
/// passed anyway all three times).
...
/// column-direct formula, with no shared helper and no `simd_quatf`/
/// `simd_float3x3` construction on the input side at all. Note this makes
/// these two tests *not* independent of `worldTransform`'s own formula in
/// the way earlier versions of this doc comment claimed — after three
/// rounds of a self-consistent-but-wrong formula surviving hand-derived
/// tests aimed at the *previous* formula, that claim of independence
/// wasn't actually delivering what it promised. The real, load-bearing
/// check against getting this wrong a fourth time lives in
/// `AdjacentTileRotationAlignmentTests` and `CoordinateSystemRegressionTests`
/// (real archive data, real mesh geometry, checking actual edge alignment
/// between real neighboring placements) — those don't depend on any
/// assumption about which direction is "correct," only on whether real
/// geometry actually connects, which is a property no self-consistent
/// wrong formula can fake.
final class SceneryModelPlacementTransformTests: XCTestCase {
    ...
    /// (corrected) formula: `worldTransform` computes `col_j` (j=0,1,2) as
    /// `row_j` directly (no transpose), and position as `(-row3.x, row3.y,
    /// row3.z)`. Inverting: `row_j = col_j` directly, `row3 =
    ...
    /// Hand-derived directly from the *current* formula (`col_j = row_j`,
    /// no transpose) — see this file's own top-level doc comment for why
    /// this is no longer claimed to be independent of `worldTransform`
    /// itself. A 90° rotation about Y with uniform scale 2 sends local +X
    /// to world (0,0,-2), +Y to (0,2,0), +Z to (2,0,0) (standard
    /// right-handed convention, verified by hand), at world position
    /// (5,6,7). Since `col_j = row_j` directly: `row0 = col0 = (0,0,-2)`,
    /// `row1 = col1 = (0,2,0)`, `row2 = col2 = (2,0,0)`, and position
    /// `(5,6,7) = (-row3.x, row3.y, row3.z)` gives `row3 = (-5,6,7)`.
    ...
    /// `row_j = col_j` directly: `row0=(0,3,0)`, `row1=(-1,0,0)`,
    /// `row2=(0,0,1)`.
    func testNinetyDegreeZRotationWithNonUniformScaleMatchesHandComputedRawBytes() throws {
        let row0 = SIMD4<Float>(0, 3, 0, 0)
        let row1 = SIMD4<Float>(-1, 0, 0, 0)
