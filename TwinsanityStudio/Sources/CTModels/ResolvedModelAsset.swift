        ...
        if let graphics = fileRoot.children.first(where: { graphicsSectionTypes.contains($0.sectionType) }) {
            for section in graphics.children {
                for leaf in section.children {
                    ...
                    case .skeleton, .animation, .position, .instance, .trigger, .camera, .collision, .scenery, .dynamicScenery, .soundEffect, .gameObject, .chunkLinks, .aiPosition, .aiPath, .collisionSurface, .skydome, .path, .raw:
                        break
                    }
                }
            }
